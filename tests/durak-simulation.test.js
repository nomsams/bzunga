...
for (let playerCount = 2; playerCount <= 6; playerCount++) {
    ...
    while (engine.state.phase !== 'game_over' && actions < 10000) {
        ...
        assert(action, `${playerCount}-player simulation deadlocked in ${engine.state.phase}`);
...
console.log('Durak simulation: complete legal deals for 2 through 6 players passed.');
