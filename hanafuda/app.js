(function () {
    ...
    const App = {
        ...
        joinRejected: false, gameState: null,
    ...
    const Net = {
        resetJoinAttempt(resetButtons = true) {
            clearTimeout(App.peerOpenTimer); clearTimeout(App.reconnectTimer);
            App.guestLink?.stop?.(); App.guestLink = null;
            try { App.hostConnection?.close?.(); } catch (error) {}
            try { App.peer?.destroy?.(); } catch (error) {}
            App.hostConnection = null; App.peer = null; App.localId = null; App.transport = ''; App.reconnectTimer = null;
        ...
        createHost(peerId, offline = false) {
            ...
            App.localId = peerId; App.offlineHost = offline;
        ...
        connectToHost(hostId) {
            ...
            App.guestLink = RoomTools.ResilientJoin.connect({
                ...
                onReady: () => { document.getElementById('lobby-start').classList.add('hidden'); document.getElementById('lobby-room').classList.remove('hidden'); document.getElementById('client-waiting').classList.remove('hidden'); document.getElementById('room-id-display').textContent = `Connected to ${safeHostId}`; },
                onData: data => Net.receiveHostData(data),
                onDrop: () => { if (!App.leaving && !App.joinRejected) { UI.showToast('Connection dropped. Rejoining your flower cards…', 'danger'); Net.scheduleReconnect(safeHostId); } },
        ...
        acceptConnection(connection) {
            ...
            connection.on('close', () => {
                ...
                const spectator = App.connectionRoles[connection.peer]?.role === 'spectator';
                delete App.connections[connection.peer]; delete App.connectionRoles[connection.peer]; delete App.spectators[connection.peer];
                spectator ? Net.broadcast() : Game.engine?.disconnectPlayer(connection.peer);
            });
        },
        ...
        handleJoin(connection, data) {
            ...
            if (sameSeat) { App.connections[connection.peer] = connection; App.connectionRoles[connection.peer] = { role: 'player' }; if (!sameSeat.connected) Game.engine.reconnectPlayer(sameSeat.id, connection.peer); Net.sendState(connection, connection.peer); return; }
            ...
            if (returning) { App.connections[connection.peer] = connection; App.connectionRoles[connection.peer] = { role: 'player' }; Game.engine.reconnectPlayer(returning.id, connection.peer); Net.sendState(connection, connection.peer); return; }
            ...
            App.connections[connection.peer] = connection; App.connectionRoles[connection.peer] = { role: 'player' };
        ...
        receiveHostData(data) {
            if (data?.type === 'STATE_UPDATE' && data.state?.players && data.state?.field) {
                App.isSpectator = Boolean(data.state.spectatorMode); if (App.isSpectator && data.state.viewerId) App.localId = data.state.viewerId;
        ...
        sendState(connection, peerId) {
            ...
            const perspectiveId = spectator ? role.perspectiveId : peerId;
    ...
    const UI = {
        ...
        renderQr(peerId) { const container = document.getElementById('qr-container'); RoomTools.renderQr(container, RoomTools.inviteUrl('hanafuda', peerId), 220); RoomTools.bindQr(container); },
        ...
        startGame() {
            if (!App.isHost || !Game.engine) return;
            const result = Game.engine.startGame({ rounds: Number(document.getElementById('round-count').value), viewingYaku: document.getElementById('viewing-yaku').checked, bustedViewing: document.getElementById('busted-viewing').checked, cardBack: document.getElementById('card-back-select').value });
            if (!result.ok) UI.showToast(result.reason, 'danger');
        },

        render(state) {
            App.gameState = state; UI.setCardBack(state.settings?.cardBack || document.documentElement.dataset.hanafudaBack); UI.renderLogs(state);
        ...
        renderResult(state) {
            const modal = document.getElementById('result-modal'); if (!['END_ROUND', 'MATCH_OVER'].includes(state.phase) || App.ui.dismissedRound === state.roundNumber) { modal.classList.add('hidden'); return; }
            ...
            if (App.isHost && !matchOver) { const button = document.createElement('button'); button.id = 'btn-next-round'; button.className = 'primary'; button.textContent = 'DEAL NEXT MONTH'; button.onclick = () => { App.ui.dismissedRound = state.roundNumber; Net.sendAction({ type: 'START_NEXT_ROUND' }); }; document.getElementById('result-actions').prepend(button); }
            document.getElementById('modal-overlay').classList.remove('hidden'); modal.classList.remove('hidden');
        },
        hideResult() { if (App.gameState) App.ui.dismissedRound = App.gameState.roundNumber; document.getElementById('result-modal').classList.add('hidden'); if (document.getElementById('rules-modal').classList.contains('hidden') && document.getElementById('capture-modal').classList.contains('hidden')) document.getElementById('modal-overlay').classList.add('hidden'); },
        animateAction(state) {
        ...
        renderLogs(state) {
            ...
            const box = document.getElementById('chat-messages'); box.innerHTML = state.logs.map(log => log.type === 'chat' ? `<div class="chat-line"><strong>${Utils.escape(log.name)}:</strong> ${Utils.escape(log.message)}</div>` : `<div class="chat-line system">${Utils.escape(log.message)}</div>`).join(''); box.scrollTop = box.scrollHeight;
        },
